...
<script>
    ...
    //-------------------------------------------------------------------------------------------------
        ...
        $.ajax({
            method: "GET",
            url: "{{ route('JabatanBidang.detail') }}",
            data: {
                id: st_id
            },
            success: function (response) {
                if (response.status == 404) {
                    ...
                    $('#d_jabatan').text(response.jabatanBidang.Jabatan);  // Perbaiki penanganan respons
                    $('#d_bidang').text(response.jabatanBidang.namaBidang);
                    $('#d_nama_jabatan_bidang').text(response.jabatanBidang.namaJabatanBidang);
                    $('#d_keterangan').text(response.jabatanBidang.keterangan);
                }
            },
    ...
    $(document).on('click', '.delete_data', function (e) {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('JabatanBidang.delete') }}",
            data: data,
            dataType: "json",
            success: function (response) {
        ...
        <div class="card custom-card">
            <div class="card-header justify-content-between">
                <div class="card-title">
                    Daftar Jabatan Bidang
                </div>
                <div class="prism-toggle">
                    <a class="btn btn-primary btn-wave waves-effect waves-light" href="{{ route('JabatanBidang.create') }}">
                        <i class="ri-add-line align-middle me-1 fw-medium"></i> Tambah Jabatan Bidang
                    </a>
                </div>
            </div>
            <div class="card-body">
                <table id="responsiveDataTable" class="table table-bordered text-nowrap w-100">
                    <thead>
                        <tr>
                            <th>Jabatan</th>
                            <th>Bidang</th>
                            <th>Nama Jabatan Bidang</th>
                            <th>Keterangan</th>
                            <th class="text-center" style="width: 10px;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if (isset($jabatanbidang) && count($jabatanbidang) > 0)
                            @foreach ($jabatanbidang as $st)
                                <tr>
                                    <td>{{ $st->Jabatan }}</td>
                                    <td>{{ $st->namaBidang }}</td>
                                    <td>{{ $st->namaJabatanBidang }}</td>
                                    <td>{{ $st->keterangan }}</td>
                                    <td>
                                        <div class="dropdown">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Detail Jabatan Bidang</h6>
            ...
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        ...
                        <span class="d-block fs-13 text-muted fw-normal" id="d_jabatan"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        <h6 class="mb-1 fs-13">Bidang</h6>
                        <span class="d-block fs-13 text-muted fw-normal" id="d_bidang"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        <h6 class="mb-1 fs-13">Nama Jabatan Bidang</h6>
                        <span class="d-block fs-13 text-muted fw-normal" id="d_nama_jabatan_bidang"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
