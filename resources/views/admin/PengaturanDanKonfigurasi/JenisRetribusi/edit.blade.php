...
<div class="row">
    <div class="col-xl-12">
    @if (isset($retribusi))
        <form action="{{ route('JenisRetribusi.update', $retribusi -> idJenisRetribusi)}}" method="post" class="row g-3 needs-validation" novalidate>
        {{ csrf_field() }}
        <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        Ubah Jenis Retribusi
                    </div>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="validationCustom01" class="form-label">Jenis Retribusi</label>
                        <input type="text" class="form-control" id="validationCustom01" placeholder="Masukkan Jenis Retribusi" name="JenisRetribusi" value="{{ $retribusi -> JenisRetribusi }}" required>
                        <div class="invalid-feedback">
                            Jenis Retribusi Tidak Boleh Kosong
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="validationTextarea" class="form-label">Keterangan</label>
                        <textarea class="form-control" id="validationTextarea" placeholder="Masukkan Keterangan" rows="4" name="Keterangan">{{ $retribusi -> Keterangan }}</textarea>
                    </div>
