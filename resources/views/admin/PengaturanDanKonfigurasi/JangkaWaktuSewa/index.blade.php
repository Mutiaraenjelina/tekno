...
<script>
    ...
    //-------------------------------------------------------------------------------------------------
        ...
        $.ajax({
            method: "GET",
            url: "{{ route('JangkaWaktuSewa.detail') }}",
            data: {
                id: st_id
            },
            success: function (response) {
                if (response.status == 404) {
                    ...
                    $('#d_jangka_waktu_sewa').text(response.status.jangkaWaktu);  // Perbaiki penanganan respons
                    $('#d_jenis_jangka_waktu').text(response.status.jenisJangkaWaktu);
    ...
    $(document).on('click', '.delete_data', function (e) {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('JangkaWaktuSewa.delete') }}",
            data: data,
            dataType: "json",
            success: function (response) {
                if (response.status == 400) {
                    ...
                    setTimeout(function() {
                        window.location='{{ route('JangkaWaktuSewa.index') }}';
                    }, 2500);
                }
            }
        });
        ...
        <div class="card custom-card">
            <div class="card-header justify-content-between">
                <div class="card-title">
                    Daftar Jangka Waktu Sewa
                </div>
                <div class="prism-toggle">
                    <a class="btn btn-primary btn-wave waves-effect waves-light" href="{{ route('JangkaWaktuSewa.create') }}">
                        <i class="ri-add-line align-middle me-1 fw-medium"></i> Tambah Jangka Waktu Sewa
                    </a>
                </div>
            </div>
            <div class="card-body">
                <table id="responsiveDataTable" class="table table-bordered text-nowrap w-100">
                    ...
                    <tbody>
                        @if (isset($jangkaWaktuSewa) && count($jangkaWaktuSewa) > 0)
                            @foreach ($jangkaWaktuSewa as $st)
                                <tr>
                                    <td>{{ $st->jenisJangkaWaktu }}</td>
                                    <td>{{ $st->jangkaWaktu }}</td>
                                    <td>{{ $st->keterangan }}</td>
                                    <td>
                                        <div class="dropdown">
            ...
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        ...
                        <span class="d-block fs-13 text-muted fw-normal" id="d_jangka_waktu_sewa"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
