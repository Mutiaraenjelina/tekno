...
<script>
    ...
    $(document).on('click', '.detailBtn', function (e) {
        ...
        $.ajax({
            method: "GET",
            url: "{{ route('JenisJangkaWaktu.detail') }}",
            data: {
                id: st_id
            },
            success: function (response) {
                ...
                if (response.status == 404) {
                    ...
                    $('#d_jenis_jangka_waktu').text(response.jenisJangkaWaktu.jenisJangkaWaktu);
                    $('#d_keterangan').text(response.jenisJangkaWaktu.keterangan);
                }
            }
        });
    });
    ...
    $(document).on('click', '.delete_data', function (e) {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('JenisJangkaWaktu.delete') }}",
            data: data,
            dataType: "json",
            success: function (response) {
        ...
        <div class="card custom-card">
            ...
            <div class="card-body">
                <table id="responsiveDataTable" class="table table-bordered text-nowrap w-100">
                    ...
                    <tbody>
                        @if (isset($JangkaWaktuType) && count($JangkaWaktuType) > 0)
                            @foreach ($JangkaWaktuType as $st)
                                <tr>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <button type="button" value="{{ $st->idjenisJangkaWaktu }}" class="dropdown-item detailBtn">
                                                        <i class="ri-eye-line me-1 align-middle d-inline-block"></i>Detail
                                                    </button>
                                                </li>
                                                <li><a class="dropdown-item" href="{{ route('JenisJangkaWaktu.edit', $st->idjenisJangkaWaktu) }}">
                                                    <i class="ri-edit-line me-1 align-middle d-inline-block"></i>Ubah</a>
                                                </li>
                                                <li><button type="button" value="{{ $st->idjenisJangkaWaktu }}" class="dropdown-item deleteBtn">
