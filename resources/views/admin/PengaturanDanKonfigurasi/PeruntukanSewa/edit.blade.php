...
<div class="row">
    <div class="col-xl-12">
        <form class="row g-3 needs-validation" action="{{ route('PeruntukanSewa.update', $peruntukanSewa -> idperuntukanSewa)}}" method="post" novalidate>
        {{ csrf_field() }}      
        <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        Ubah Peruntukan Sewa
                    </div>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="js-example-placeholder-single form-control" name="jenisKegiatan" required>
                            ...
                            @foreach ($peruntukanSewaCombo as $sT)
                                <option value="{{ $sT->idjenisKegiatan }}" {{ $sT->idjenisKegiatan === $peruntukanSewa->idjenisKegiatan ? 'selected' : '' }}>
                                    {{ $sT->jenisKegiatan }}
                                </option>
                            @endforeach
                        </select>                        
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="validationCustom01" placeholder="Masukkan Status" name="peruntukanSewa"
                            value="{{ $peruntukanSewa->peruntukanSewa }}" required>
                        <div class="invalid-feedback">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="validationTextarea" placeholder="Masukkan Keterangan" name="keterangan"
                            rows="4">{{ $peruntukanSewa->keterangan }}</textarea>
                    </div>
