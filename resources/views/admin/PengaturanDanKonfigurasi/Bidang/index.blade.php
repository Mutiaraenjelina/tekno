        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 404) {
                    ...
                    $('#d_parentBidang').text(response.bidang.parentBidang);
                    $('#d_namaBidang').text(response.bidang.namaBidang);
                    $('#d_namaDepartmen').text(response.bidang.namaDepartmen);
                    $('#d_keterangan').text(response.bidang.keterangan);
                }
            }
        });
    });
        ...
        <div class="card custom-card">
            <div class="card-header justify-content-between">
                ...
                <div class="prism-toggle">
                    <a class="btn btn-primary btn-wave waves-effect waves-light" href="{{ route('Bidang.create') }}">
                        <i class="ri-add-line align-middle me-1 fw-medium"></i> Tambah Bidang
                    </a>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @if (isset($bidang) && count($bidang) > 0)
                            @foreach ($bidang as $bdg)
                                <tr>
                                    <td>{{ $bdg->namaDepartmen }}</td>
                                    <td>{{ $bdg->parentBidang }}</td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Detail Bidang</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        <h6 class="mb-1 fs-13">Departmen</h6>
                        <span class="d-block fs-13 text-muted fw-normal" id="d_namaDepartmen"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        <h6 class="mb-1 fs-13">Parent Bidang</h6>
                        <span class="d-block fs-13 text-muted fw-normal" id="d_parentBidang"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        <h6 class="mb-1 fs-13">Nama Bidang</h6>
                        <span class="d-block fs-13 text-muted fw-normal" id="d_namaBidang"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Hapus Data Bidang</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="deleteBidangForm">
                @csrf
