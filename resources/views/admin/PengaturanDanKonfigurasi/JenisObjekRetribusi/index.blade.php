...
<script>
    ...
    $(document).on('click', '.detailBtn', function (e) {
        ...
        $.ajax({
            method: "GET",
            url: "{{ route('JenisObjekRetribusi.detail') }}",
            data: {
                id: st_id
            },
            success: function (response) {
                if (response.status == 404) {
                    ...
                    $('#d_jenis_objek_retribusi').text(response.objekRetribusi.jenisObjekRetribusi);
                    $('#d_keterangan').text(response.objekRetribusi.keterangan);
                }
            }
        });
    });
    ...
    $(document).on('click', '.delete_data', function (e) {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('JenisObjekRetribusi.delete') }}",
            data: data,
            dataType: "json",
            success: function (response) {
                if (response.status == 400) {
                    ...
                    setTimeout("window.location='{{ route('JenisObjekRetribusi.index') }}'", 1200);
                }
            }
        });
    });
...
<div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
    <div>
        <h1 class="page-title fw-medium fs-18 mb-2">Jenis Objek Retribusi</h1>
        <div class="">
            <nav>
                <ol class="breadcrumb breadcrumb-example1 mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Jenis Objek Retribusi</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="card custom-card">
            <div class="card-header justify-content-between">
                <div class="card-title">
                    Daftar Jenis Objek Retribusi
                </div>
                <div class="prism-toggle">
                    <a class="btn btn-primary btn-wave waves-effect waves-light"
                        href="{{ route('JenisObjekRetribusi.create') }}">
                        <i class="ri-add-line align-middle me-1 fw-medium"></i> Tambah Jenis Objek Retribusi
                    </a>
                </div>
            </div>
            <div class="card-body">
                <table id="responsiveDataTable" class="table table-bordered text-nowrap w-100">
                    ...
                    <tbody>
                        @if (isset($objekRetribusi) && count($objekRetribusi) > 0)
                            @foreach ($objekRetribusi as $jor)
                                <tr>
                                    <td>{{ $jor->jenisObjekRetribusi }}</td>
                                    <td>{{ $jor->keterangan }}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" style="">
                                                <li>
                                                    <button type="button" value="{{ $jor->idJenisObjekRetribusi }}"
                                                        class="dropdown-item detailBtn">
                                                        <i class="ri-eye-line me-1 align-middle d-inline-block"></i>Detail
                                                    </button>
                                                </li>
                                                <li><a class="dropdown-item"
                                                        href="{{ route('JenisObjekRetribusi.edit', $jor->idJenisObjekRetribusi) }}"><i
                                                            class="ri-edit-line me-1 align-middle d-inline-block"></i>Ubah</a>
                                                </li>
                                                <li><button type="button" value="{{ $jor->idJenisObjekRetribusi }}"
...
<!-- Start:: Detail Jenis Objek Retribusi -->
<div class="modal fade" id="detailModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Detail Jenis Objek Retribusi</h6>
            ...
            <div class="modal-body px-4">
                <div class="d-flex gap-3">
                    <div class="flex-fill">
                        ...
                        <span class="d-block fs-13 text-muted fw-normal" id="d_jenis_objek_retribusi"></span>
                    </div>
                </div>
            </div>
            <div class="modal-body px-4">
...
<!-- End:: Detail Jenis Objek Retribusi -->

<!-- Start:: Delete Jenis Objek Retribusi-->
...
<!-- End:: Delete Jenis Objek Retribusi -->
